...
import type { ContractStructure, RedeemerInfo, RedeemerVariant, ValidationCheck, FieldInfo, ScriptPurpose, DatumInfo } from './types.js';
import { detectValidator, getRedeemerParam, getDatumParam } from './validator.js';
import { analyzeDatum } from './datum.js';
...
export function analyzeContract(ast: UplcTerm): ContractStructure {
  ...
  // Get parameter names
  const datumParam = getDatumParam(validator);
  const redeemerParam = getRedeemerParam(validator);
  
  // Analyze datum patterns (spend validators only)
  const datum = analyzeDatum(validator.body, datumParam);
  ...
  return {
    ...
    bodyWithBindings: validator.bodyWithBindings,
